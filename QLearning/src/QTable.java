...
/**
    ...
    public class QTable {
        ...
        int getBestAction(char[] map){
        	float[] currQvalues = getValues(map);
        	if(currQvalues == null) 
        	{
        		return 0;
        	}
        	...
        	{
        		System.out.println("#############--------------PANIC: GREATER THAN THREE-------------------#############");
        	}
        	return index;
            //return 0;
        }
        ...
        void updateQvalue(int reward, char[] map){
        	...
        	if(getMapString(map).equals(getMapString(prevState))) 
        	{
        		return;
        	}
        	...
        	float[] prevQvalues = getValues(prevState);
        	 ...
        	 if(prevQvalues == null) 
             {
         		prevQvalues = new float[actionRange];
         		for(int i = 0; i<actionRange; i++)
         		{
         			prevQvalues[i] = 0;
         		}
         		
         		String mapKey = getMapString(prevState);
         		table.put(mapKey, prevQvalues);
         		return;
             }
        	
        	System.out.println("Prev action " + prevAction);
        	System.out.println("Prev values len " + prevQvalues.length);
        	...
        	if(currQvalues == null) 
            {
        		currQvalues = new float[actionRange];
         		for(int i = 0; i<actionRange; i++)
         		{
         			currQvalues[i] = 0;
         		}
        	...
        	float gain = learningRate * (reward + gammaValue * currQvalue - prevQValue);
        	float updatedQValue = prevQValue + gain;
        	
        	//Update
        	prevQvalues[prevAction] = updatedQValue;
        	table.put(getMapString(prevState), prevQvalues);
